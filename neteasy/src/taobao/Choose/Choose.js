...
class Choose extends React.Component {
  ...
  handleClick(e) {
    this.setState({
      ...
      control: (!this.state.show == true ? '显示筛选' : '隐藏筛选'),
    });
  }

  handleChoose(e) {
    this.setState({
      checkbox: e.target.dataset.id,
    });
    e.preventDefault();
  }
  ...
  render() {
    return (
      <div className='choose'>
        <div className='open'>
          <span>所有分类</span>
          <span onClick={(e) => this.handleClick(e)}>{this.state.control}</span>
        </div>
        <div style={this.state.control == '隐藏筛选' ? { maxHeight: '250px' } : { maxHeight: 0, border: 'none' }} className='content'>
          {this.state.checkbox == '选购热点：' ?
            <CheckBox
              list={firstCheckBox}
            ...
            <List
              ...
              cssDiv='cssDiv'
            />}
          {this.state.checkbox == '材质：' ?
            <CheckBox
              list={secondCheckBox}
              onclick={(e) => this.handleChoose(e)}
              name='checket' content='材质：'
              cssForm='cssForm'
        ...
        <div>
          <Condition
            ...
            cssName='condition'
            price='price'
            addressCss='addressCss'
            serve={serve}
            priceSort={(e) => this.priceSort(e)}
            range={(e) => this.range(e)}
            addressRange={(e) => this.addressRange(e)}
          />
        </div>
        <div>
          <div className='serve'>
